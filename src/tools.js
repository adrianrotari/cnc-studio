...
const SEED_TOOLS = [
  ...
  },
  {
    id:'inova-247121-10',
    type:'endmill',
    name:'Inova 247 HPC ø12 (247.121.10)',
    vendor:'Inova Tools',
    orderNo:'247.121.10',
    dc:12, z:3, lc:26, reach:73, neckDia:11.7, shankDia:12, oal:120,
    cornerR:0.2, cornerType:'chamfer 0.2x45', throughCoolant:true, coolantStyle:'IC',
    note:'247 series solid-carbide HPC with internal coolant, long version L1 120 / L3 73; .10 = shank variant (confirm HA/HB); helix + coating not captured yet',
    // dims verified against the inovatools.eu 247 series table; silhouette measured
    // from the vendor STEP's circle entities
    profile:[[0,5.8],[0.2,6],[26,6],[26.56,5.85],[72.74,5.85],[73.56,6],[118.8,6],[120,4.8]]
  },
  {
...
function validateTool(t){
  ...
  if(type==='endmill'){
    ...
    if(t.cut!=null){                    // cut data is optional (may be uncaptured)
      if(typeof t.cut!=='object') p.push('cut must be an object');
