...
function parseNC(text){
  ...
  const addSeg=(pts,kind,lineNo,srcTxt)=>{
    ...
    cur.segs.push({pts,kind,line:lineNo,f:st.f,s:st.s,len,src:srcTxt});
  ...
  for(let li=0; li<lines.length; li++){
    const raw=lines[li];
    let code=raw.replace(/\(([^)]*)\)/g,(m,c)=>{ if(cur&&cur.desc.length<6&&c.trim())cur.desc.push(c.trim()); return ' '; });
    code=code.trim();
    if(!code) continue;
    ...
    if(mN){
      const cm = raw.match(/\(([^)]*)\)/);
      newSec('N'+mN[1]+(cm?' '+cm[1].trim():''), li+1);
    ...
    if(ms.includes(98) || gs.includes(65)){
      const p=w.P; if(p!==undefined) cur.calls.push((ms.includes(98)?'O':'macro P')+p);
      continue;
    }
    if(gs.includes(52)){
    ...
    if(cycle!==null){
      ...
      if(axis && st.x!==null && st.y!==null && st.z!==null){
        ...
        if(pre.some((v,i)=>Math.abs(v-p0[i])>1e-9)) addSeg([p0,pre],'rapid',li+1,raw.trim());
        addSeg([pre,[tgt.x,tgt.y,tgt.z]],'cycle',li+1,raw.trim());
        addSeg([[tgt.x,tgt.y,tgt.z],pre],'cycle',li+1,raw.trim());
    ...
    if(ready){
      if(st.motion===2||st.motion===3){
        const arc=arcPts(dest,w,st.motion===2,li+1);
        if(arc) addSeg(arc.pts, 'feed', li+1, raw.trim());
        else addSeg([[st.x,st.y,st.z],[dest.x,dest.y,dest.z]],'feed',li+1,raw.trim());
      } else {
        addSeg([[st.x,st.y,st.z],[dest.x,dest.y,dest.z]],
               st.motion===0?'rapid':'feed', li+1, raw.trim());
      }
    }
